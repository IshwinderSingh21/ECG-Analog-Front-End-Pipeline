...
To fix this, this circuit implements **active bootstrapping** via `BOOTSTRAP_BUFFER2`. By feeding 90% of the output signal back into the center of the filter using the active `boot` node network, we block current leakage at surrounding frequencies. This drastically sharpens the filter's selectivity (Q-factor), keeping the 10 Hz passband completely flat while creating a razor-sharp drop at exactly 50 Hz.
